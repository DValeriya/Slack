...
def get_messages():
    ...
    for message, item in zip(message_history["messages"], response):
        ...
        if "thread_ts" in message:
            ...
            item["thread"] = [
                {response_key: thread_message[request_key] for response_key, request_key in
                 zip(response_keys, request_keys)}
        ...
        for thread in item["thread"]:
            ...
            thread["sender"] = slack.users_info(token=slack_info.bot_access_token, user=thread["sender"])["user"][
                "name"]
